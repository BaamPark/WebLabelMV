...
def build_contextual_chat_prompt(project, image_relationship_text=None,
    ...
    if boxes_for_target_frame is not None:
        payload['boxes_for_target_frame'] = boxes_for_target_frame

    if has_selected_box_crop:
        image_order = (
            "Image order:\n"
            "- Image 1: cropped image of the selected box from the current frame\n"
        )
    else:
    ...
    action_instructions = (
        "If the user asks you to create new annotation boxes from the frame, call create_box with the target class only; "
        "the backend will run object-counting and box-detection subagents before creating boxes. "
        ...
        "Box creation example: ACTION_JSON: {\"action\":\"create_box\",\"target_frame\":\"current\",\"className\":\"clinician\"}. "
        "Edit example: ACTION_JSON: {\"action\":\"update_box_object_id\",\"target_frame\":\"current\",\"box_id\":\"x82lq2\",\"objectId\":1}. "
        "For the supported box creation tool, use this schema: "
        "{\"action\":\"create_box\",\"target_frame\":\"current\",\"className\":\"<project class>\"}. "
        ...
        "the backend will spawn one metadata-generated subagent to classify all project attributes for the selected box. "
